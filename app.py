...
def clasificar_grupos(df):
    ...
    condiciones = [
        (df_clasificado['RADIO_HIDRAULICO'] > 0.5),  # GRUPO_ALTO_RH (umbral más bajo)
        (df_clasificado['YEAR'] >= 2020),            # GRUPO_RECIENTE (más flexible)
    ]
...
def ajustar_curva_grupo(datos_grupo, nombre_grupo):
    try:
        ...
        if len(H) < 2:  # Reducido a 2 puntos mínimo
            st.warning(f"   ⚠️ {nombre_grupo}: Solo {len(H)} puntos")
            return None
        ...
        for nombre, funcion in modelos:
            try:
                ...
                if nombre == 'Potencial':
                    ...
                    params, _ = curve_fit(funcion, H_pos, Q_pos, p0=[1.0, 1.5], maxfev=5000)
                ...
                # Aceptar modelos con R² muy bajo o incluso negativo (mejor que nada)
                if r2 > mejor_r2:
                    mejor_r2 = r2
                    mejor_modelo = {
                        'nombre': nombre,
        ...
        if mejor_modelo:
            ...
            st.warning(f"   ❌ {nombre_grupo}: No se pudo ajustar ningún modelo")
            return None
...
def procesar_con_clasificacion(df, incluir_alto_rh=True):
    ...
    with st.spinner("Ajustando curvas..."):
        ...
        for grupo in df_filtrado['GRUPO_PREDICHO'].unique():
            ...
            if len(grupo_data) >= 2:  # Solo 2 puntos mínimo
                curva = ajustar_curva_grupo(grupo_data, grupo)
                if curva:
                    resultados[grupo] = curva
            else:
                st.warning(f"⚠️ {grupo}: Solo {len(grupo_data)} puntos")
