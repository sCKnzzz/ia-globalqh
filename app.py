...
def ajustar_curva_grupo(datos_grupo, nombre_grupo):
    try:
        ...
        debug_text += f"- Niveles: {H}\n"
        debug_text += f"- Caudales: {Q}\n"
        debug_text += f"- Rango niveles: {min(H):.2f} - {max(H):.2f} m\n"
        debug_text += f"- Rango caudales: {min(Q):.2f} - {max(Q):.2f} m³/s\n"
        st.write(debug_text)
        ...
        if np.std(H) < 0.01 or np.std(Q) < 0.01:
            st.warning(f"⚠️ {nombre_grupo}: Poca variación en los datos")
        ...
        for nombre, funcion in modelos:
            try:
                ...
                # Aceptar CUALQUIER modelo que converja (incluso con R² negativo)
                if r2 > mejor_r2:
                    mejor_r2 = r2
                    mejor_modelo = {
                        'nombre': nombre,
