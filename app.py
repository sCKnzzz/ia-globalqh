...
# FUNCIÓN PARA PREPARAR DATOS (CORREGIDA)
def preparar_datos(df):
    ...
    # VERIFICAR Y CALCULAR PERÍMETRO DE FORMA MÁS ROBUSTA
    necesita_calcular_perimetro = False
    
    # Verificar si la columna PERIMETRO existe
    if 'PERIMETRO' not in df_procesado.columns:
        necesita_calcular_perimetro = True
        df_procesado['PERIMETRO'] = np.nan  # Crear columna si no existe
    else:
        # Verificar si hay valores NaN, cero o negativos
        if df_procesado['PERIMETRO'].isna().any() or (df_procesado['PERIMETRO'] <= 0).any():
            necesita_calcular_perimetro = True
    
    # Calcular perímetro si es necesario
    if necesita_calcular_perimetro:
        # Calcular tirante medio (Area / Ancho)
        df_procesado['TIRANTE_MEDIO'] = df_procesado['AREA'] / df_procesado['ANCHO_RIO']
        
        # Estimar perímetro usando aproximación rectangular
        # Para sección rectangular: Perímetro = Ancho + 2 * Tirante
        df_procesado['PERIMETRO'] = df_procesado['ANCHO_RIO'] + 2 * df_procesado['TIRANTE_MEDIO']
        
        st.info(f"📏 Perímetro calculado automáticamente para {len(df_procesado)} aforos usando aproximación rectangular")
        st.write(f"📐 Fórmula: Perímetro = Ancho + 2 × (Área/Ancho)")
        st.write(f"📊 Rango de perímetros calculados: {df_procesado['PERIMETRO'].min():.2f} - {df_procesado['PERIMETRO'].max():.2f} m")
    ...
    for i in range(num_aforos):
        with st.expander(f"Aforo {i+1}", expanded=True if i == 0 else False):
            ...
            datos_manual.append({
                ...
                'ANCHO RIO (m)': ancho,
                'VELOCIDAD (m/s)': velocidad
                # PERIMETRO se calculará automáticamente
            })
